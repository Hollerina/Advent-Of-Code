...
const increasingNumbers = (numberArray: string[]) => {
  ...
  for (let i = 1; i < numberArray.length - 1; i++) {
    if (
      Number(numberArray[i + 1]) - Number(numberArray[i]) > 3 ||
      Number(numberArray[i]) >= Number(numberArray[i + 1])
    ) {
      state = 'NotSafe'
      break
    }
...
const decreasingNumbers = (numberArray: string[]) => {
  ...
  for (let i = 1; i < numberArray.length - 1; i++) {
    if (
      Number(numberArray[i]) - Number(numberArray[i + 1]) > 3 ||
      Number(numberArray[i]) <= Number(numberArray[i + 1])
    ) {
      state = 'NotSafe'
      break
...
input.forEach((numbers) => {
  ...
  let state: string = ''
  if (
    Number(splitNumbers[0]) < Number(splitNumbers[1]) &&
    Number(splitNumbers[1]) - Number(splitNumbers[0]) <= 3
  ) {
    state = increasingNumbers(splitNumbers)
  } else if (
    Number(splitNumbers[0]) > Number(splitNumbers[1]) &&
    Number(splitNumbers[0]) - Number(splitNumbers[1]) <= 3
  ) {
